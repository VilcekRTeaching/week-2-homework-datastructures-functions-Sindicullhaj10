...
```{r}
##install.packages("") to install a package and to use one is library()
```
...
```{r}
vars <- nyc[ , c("CADMIUM", "LEAD", "CHOLESTEROLTOTAL", "GLUCOSESI", "CREATININESI", "CREATININE")]

apply(vars,2,median,na.rm=TRUE)
```
...
for (col in cols) {
  sds <- c(sds, sd(nyc[[col]], na.rm = TRUE))
}
